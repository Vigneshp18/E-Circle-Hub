...
<br>
<h2 style="text-align:center; color: #018786; font-family: cursive;">Announcements</h2>
<div class="container container-center">
<?php
  $dbserver="localhost";
  $dbuser="id17115978_sinehan001";
  $dbpass="@Sinehandatabase001";
  $dbname="id17115978_courses";
  $conn=mysqli_connect($dbserver,$dbuser,$dbpass,$dbname); 
  $res=mysqli_query($conn,'SELECT title,img from udemy WHERE content="Code your First Game : Arcade Classic in Javascript on Canvas"');
  $result=mysqli_query($conn,"SELECT * from udemy ORDER by id DESC");
       if($rows=mysqli_fetch_assoc($res))
       {
           echo'<a style="text-decoration:none;" href="https://www.udemy.com/"><img style="width: 50px;" src="'.$rows['img'].'"> <h2 style="color:#018786;"> '.$rows['title'].' </h2></a>';
       }
       while($row = mysqli_fetch_assoc($result))
       {
        echo'    <h4 style="margin-top: 8%;margin-left: 10%;margin-right:10%;"> '.$row['content'].'</h4><a href="'.$row['links'].'"> <span style="word-break: break-all;">'.$row['links'].'</span></a>';
       }
?>
</div>
<div class="container container-center">
<?php
  ...
  $dbname="id17115978_courses";
  $conn=mysqli_connect($dbserver,$dbuser,$dbpass,$dbname); 
  $res=mysqli_query($conn,'SELECT title,img from coursera WHERE content="Machine Learning"');
  $result=mysqli_query($conn,"SELECT * from coursera ORDER by id DESC");
       if($rows=mysqli_fetch_assoc($res))
       {
           echo'<a style="text-decoration:none;" href="https://www.coursera.org/in"><img style="width: 50px;" src="'.$rows['img'].'"> <h2 style="color:#018786;"> '.$rows['title'].' </h2></a>';
       }
       while($row = mysqli_fetch_assoc($result))
       {
        echo'<h4 style="margin-top: 8%;margin-left: 10%;margin-right:10%;"> '.$row['content'].'</h4><a href="'.$row['links'].'"> <span style="word-break: break-all;">'.$row['links'].'</span></a>';
